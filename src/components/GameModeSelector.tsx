...
interface GameModeSelectorProps {
  onSelectMode: (mode: 'simple' | 'campaign') => void;
}
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-[#e8f7b2] via-[#f7e8b2] to-[#b2f7ef] relative overflow-hidden p-4">
      ...
      <div className="relative z-10 w-full max-w-3xl mx-auto flex flex-col items-center">
        <div className="bg-white/90 rounded-3xl shadow-2xl p-12 w-full flex flex-col items-center border-4 border-[#8cb43a]">
          ...
          <p className="text-xl text-[#263c2a] text-center mb-8 font-semibold">Choose Your Game Mode</p>
          <div className="flex flex-row gap-8 w-full justify-center">
            <Button onClick={() => onSelectMode('simple')} className="flex flex-col items-center justify-center bg-[#8cb43a] hover:bg-[#7ba034] text-white font-bold text-2xl rounded-2xl shadow-lg px-10 py-8 gap-2 transition-all border-4 border-[#263c2a] w-56 h-48">
              <span className="text-5xl mb-2">⏱️</span>
              Quick Play
              <span className="text-base font-semibold mt-2">10 Questions</span>
            </Button>
            <Button onClick={() => onSelectMode('campaign')} className="flex flex-col items-center justify-center bg-[#f7c873] hover:bg-[#f7b23b] text-[#263c2a] font-bold text-2xl rounded-2xl shadow-lg px-10 py-8 gap-2 transition-all border-4 border-[#263c2a] w-56 h-48">
